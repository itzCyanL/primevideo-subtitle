...
export class View {
  ...
  public display(): void {
    ...
    const webPlayerElemet = this.document.getElementsByClassName(
      View.WEB_PLAYER_ELEMENT_CLASS
    )[0];
  ...
  private createDropFilesElement(): HTMLDivElement {
    ...
    dropFileInputElement.addEventListener("change", (event: Event) => {
      ...
      dropFileEvent.data = input.files[0];
