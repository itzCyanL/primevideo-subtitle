...
export class AmazonPrimeSubtitles {
  public onLoad(document: HTMLDocument) {
    const registerHandlers = new RegisterHandlers(document);
    registerHandlers.registerHandler(
      FileHandler.EVENT_NAME,
      new FileHandler(document)
    );
    registerHandlers.registerHandler(
      SrtParserHandler.EVENT_NAME,
      new SrtParserHandler(document)
    );
    registerHandlers.registerHandler(
      PlaybackHandler.EVENT_NAME,
      new PlaybackHandler(document)
    );
    registerHandlers.registerHandler(
      CloseHandler.EVENT_NAME,
      new CloseHandler(document)
    );
